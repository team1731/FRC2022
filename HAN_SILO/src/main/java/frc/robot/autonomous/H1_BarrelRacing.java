...
public class H1_BarrelRacing extends _DelayableStrafingAutoMode {
	...
	public H1_BarrelRacing(DriveSubsystem m_robotDrive) {
		...
		try {
			...
			// Path traj1Path =
			// Filesystem.getDeployDirectory().toPath().resolve(trajectoryJSON1);
			// trajectory1 = TrajectoryUtil.fromPathweaverJson(traj1Path);
			// Path traj2Path =
			// Filesystem.getDeployDirectory().toPath().resolve(trajectoryJSON2);
			// trajectory2 = TrajectoryUtil.fromPathweaverJson(traj2Path);
			// Path traj3Path =
			// Filesystem.getDeployDirectory().toPath().resolve(trajectoryJSON3);
			// trajectory3 = TrajectoryUtil.fromPathweaverJson(traj3Path);
			// Path traj4Path =
			// Filesystem.getDeployDirectory().toPath().resolve(trajectoryJSON4);
