		...
		SequentialCommandGroup commandGroup = new SequentialCommandGroup(new WaitCommand(getInitialDelaySeconds()),
				...
				createSwerveCommand(m_robotDrive, "Slalom: start zone to 1st cross", TrajectoryDirection.REV,
						TrajectoryHeading.CONVERT_TO_METERS, 0, new double[][] { // these are INCHES
								{ 0, 0, -90 }, // NOTE: robot starts with its +x (longitudinal) axis aligned with field
												// +x axis (facing the right side)
								{ 0, -30 }, { 10, -45 }, { 30, -60, 0 } // 1st crisscross
						}),

				createSwerveCommand(m_robotDrive, "Slalom: 1st cross to 2nd cross", TrajectoryDirection.REV,
						TrajectoryHeading.CONVERT_TO_METERS, 0, new double[][] { // these are INCHES
								{ 30, -60, 0 }, // NOTE: robot starts with its +x (longitudinal) axis aligned with field
												// +x axis (facing the right side)
								{ 50, -75 }, { 60, -150 }, { 50, -225 }, { 30, -240, 0 } // 2nd crisscross
						}),

				createSwerveCommand(m_robotDrive, "Slalom: 2nd cross around 2nd cross", TrajectoryDirection.REV,
						TrajectoryHeading.CONVERT_TO_METERS, 0, new double[][] { // these are INCHES
								{ 30, -240, 0 }, // NOTE: robot starts with its +x (longitudinal) axis aligned with
													// field +x axis (facing the right side)
								{ 10, -255 }, { 10, -310 }, { 30, -315 }, { 60, -310 }, { 60, -225 }, { 30, -240, 0 } // 2nd
																														// crisscross
																														// around
																														// to
																														// 2nd
