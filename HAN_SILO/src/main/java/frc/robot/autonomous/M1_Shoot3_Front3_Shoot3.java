...
public class M1_Shoot3_Front3_Shoot3 extends _DelayableStrafingAutoMode {
	public M1_Shoot3_Front3_Shoot3(DriveSubsystem m_robotDrive, IntakeSubsystem m_intake, SequencerSubsystem m_sequence,
		...
		SequentialCommandGroup commandGroup = new SequentialCommandGroup(new WaitCommand(getInitialDelaySeconds()),
				...
				// SHOOT 3
				new InstantCommand(m_shootclimb::enableShooting, m_shootclimb).withTimeout(4), new WaitCommand(3),
				...
				new ParallelCommandGroup(createSwerveCommand(m_robotDrive, "BACKWARD TO FRONT 3",
						TrajectoryDirection.REV, TrajectoryHeading.UNROTATE, 70, new double[][] { { -0.54, -4.74, -12 }, // initial
																															// pose
								{ -0.93, -3.45 }, // waypoint(s)
								{ -1.81, -3.3 }, { -1.96, -3.55 }, { -2.14, -3.4, 65 } } // final pose
				), new IntakeSeqCommand(m_intake, m_sequence).withTimeout(4)),

				new ParallelCommandGroup(createSwerveCommand(m_robotDrive, "STRAFE TO PICKUP FRONT 3",
