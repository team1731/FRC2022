...
public class RobotContainer {
	...
	// Joystick m_operatorController = new
	// Joystick(OIConstants.kOperatorControllerPort);
	 ...
	 */
	public RobotContainer(LedStringSubsystem m_ledstring, DriveSubsystem m_robotDrive, IntakeSubsystem m_intake,
			SequencerSubsystem m_sequencer, ShootClimbSubsystem m_shootclimb, LimeLightSubsystem m_vision) {
		...
		m_robotDrive.setDefaultCommand(
				...
				new RunCommand(() -> m_robotDrive.drive(
						...
						-m_driverController.getY(Hand.kLeft) * Math.abs(m_driverController.getY(Hand.kLeft))
								* DriveConstants.kMaxSpeedMetersPerSecond,

						// Get the y speed or sideways/strafe speed. We are inverting this because
						// we want a positive value when we pull to the left. Xbox controllers
						// return positive values when you pull to the right by default.
						-m_driverController.getX(Hand.kLeft) * Math.abs(m_driverController.getX(Hand.kLeft))
								* DriveConstants.kMaxSpeedMetersPerSecond,
	...
	private void configureButtonBindings() {
		...
		// new JoystickButton(m_operatorController, 3).whileActiveContinuous(new
		// IntakeSeqCommand(m_intake, m_sequencer));
		// new JoystickButton(m_operatorController, 14).whileActiveContinuous(new
		// SeqEjectCommand(m_intake, m_sequencer), true);
		...
		// m_shootclimb.spinShooter((m_operatorController.getRawAxis(4)+1)/2))
		// .whenInactive(() -> m_shootclimb.stopShooting());
		// .whenActive(new InstantCommand(m_shootclimb::enableShooting, m_shootclimb))
		// .whileActiveContinuous(new JoystickShooter(m_shootclimb, () ->
		// m_operatorController.getRawAxis(4)), false
		// .whileActiveContinuous(new ShootSeqCommand(m_shootclimb, m_sequencer, () ->
		// m_operatorController.getRawAxis(4)), false
		// );
		...
		// .whenInactive(() ->
		// m_robotDrive.setDriveSpeedScaler(m_operatorController.getRawAxis(5)));
		...
		// .whileActiveOnce(new InstantCommand(m_shootclimb::climbExtend,
		// m_shootclimb));
		// Climber Retract
		// new JoystickButton(m_operatorController, 2)
		// .whileActiveOnce(new InstantCommand(m_shootclimb::climbRetract,
		// m_shootclimb));

		// Activate Shooter via Operator Right Axis/Trigger
		// new HanTrigger(HanTriggers.DR_TRIG_RIGHT).whileActiveContinuous(new
		// ShootSeqCommand(m_sequencer));
		// Shooting
		...
		/*
		 * // Intake & Sequencer ejects works will button is held new
		 * JoystickButton(m_operatorController, 1) .whenHeld(new
		 * ParallelCommandGroup(new InstantCommand(m_intake::eject, m_intake), new
		 * InstantCommand(m_sequencer::reverse, m_sequencer))) .whenReleased(new
		 * ParallelCommandGroup(new InstantCommand(m_intake::retract, m_intake), new
		 * InstantCommand(m_sequencer::stop, m_sequencer)));
		 * 
		 * // Activate Intake via Operator Left Axis/Trigger //new
		 * HanTrigger(HanTriggers.OP_TRIG_LEFT).whileActiveContinuous(new
		 * IntakeSeqCommand(m_intake, m_sequencer));
		 * 
		 * // Activate Shooter via Operator Right Axis/Trigger //new
		 * HanTrigger(HanTriggers.OP_TRIG_RIGHT).whileActiveOnce(new
		 * ShootSeqCommand(m_shootclimb, m_sequencer));
		 * 
		 * // Select Shoot or Climb Mode new JoystickButton(m_operatorController,
		 * 3).whenPressed(new InstantCommand(m_shootclimb::modeClimb, m_shootclimb)); //
		 * .whenReleased(new InstantCommand(m_ShootClimbSubsystem::off, //
		 * m_ShootClimbSubsystem)); new JoystickButton(m_operatorController, 4) //
		 * .whenPressed(new InstantCommand(m_ShootClimbSubsystem::on, //
		 * m_ShootClimbSubsystem)) .whenReleased(new
		 * InstantCommand(m_shootclimb::modeShoot, m_shootclimb));
		 * 
		 * new JoystickButton(m_operatorController, 8) // .whenPressed(new
		 * InstantCommand(m_ShootClimbSubsystem::on, // m_ShootClimbSubsystem))
		 * 
		 * 
		 * .toggleWhenPressed(new ParallelCommandGroup[] { new
		 * InstantCommand(m_shootclimb::modeShoot, m_shootclimb)
		 * 
		 * });
		 */
	...
	private _NamedAutoMode createNamedAutoMode(String autoModeName) throws _NotImplementedProperlyException {
		switch (autoModeName) {
			...
			case "L1":
				return new _NamedAutoMode(new L1_EnemyPair_Front3(m_robotDrive, m_intake, m_sequencer, m_shootclimb));
			case "M1":
				return new _NamedAutoMode(
						new M1_Shoot3_Front3_Shoot3(m_robotDrive, m_intake, m_sequencer, m_shootclimb));
			case "M3":
				return new _NamedAutoMode(new M3_Shoot3_Buddy5(m_robotDrive, m_intake, m_sequencer, m_shootclimb));
			case "R1":
				return new _NamedAutoMode(new R1_WholeSide10(m_robotDrive, m_intake, m_sequencer, m_shootclimb));
			case "R2":
				return new _NamedAutoMode(
						new R2_Shoot3_FriendlyTriple(m_robotDrive, m_intake, m_sequencer, m_shootclimb));

			case "T3":
				return new _NamedAutoMode(
						new T3_DriveForwardIntakeDriveBackward(m_robotDrive, m_intake, m_sequencer, m_shootclimb));
			case "T4":
				return new _NamedAutoMode(new T4_ShootDriveForward(m_robotDrive, m_sequencer, m_shootclimb));
			case "T5":
				return new _NamedAutoMode(new T5_ShootDriveBackward(m_robotDrive, m_sequencer, m_shootclimb));

			case "H0":
				return new _NamedAutoMode(
						new H0_GalacticSearch(m_robotDrive, m_intake, m_sequencer, m_vision, m_shootclimb));
			case "H1":
				return new _NamedAutoMode(new H1_BarrelRacing(m_robotDrive));
			case "H2":
				return new _NamedAutoMode(new H2_Slalom(m_robotDrive));
			case "H3":
				return new _NamedAutoMode(new H3_Bounce(m_robotDrive));
	...
	/*
	 * public class ModeTrigger extends Trigger { HanMode mode; boolean result;
	 * public ModeTrigger (HanMode mode) { this.mode = mode; }
	 * 
	 * public boolean get() { boolean left = m_operatorController.getRawButton(1);
	 * boolean right = m_operatorController.getRawButton(12); switch (mode) { case
	 * MODE_SHOOT: result = ((!left) && (!right)); break; case MODE_CLIMB: result =
	 * (left && right); break; } //double v = operatorController.getY(Hand.kRight);
	 * return result; } }
	 */
}
