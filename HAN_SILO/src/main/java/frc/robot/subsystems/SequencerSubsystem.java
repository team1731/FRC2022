...
public class SequencerSubsystem extends SubsystemBase {
	...
	public SequencerSubsystem(LedStringSubsystem m_ledstring) {
		...
		mTalonSeq.configSelectedFeedbackSensor(FeedbackDevice.IntegratedSensor, OpConstants.kPIDLoopIdx,
		 ...
		 * 
		 * @Override public void interruptFired(int interruptAssertedMask, Object param)
		 * { //mTalonSeq.set(0); stop();
	 ...
	 */
	/*
	 * public void forward(boolean shooting) { if(shooting){
	 * mTalonSeq.setSpeed(OpConstants.kMotorSeqFwdShootSpeed); } else{
	 * mTalonSeq.setSpeed(OpConstants.kMotorSeqFwdIntakeSpeed); } //mPowerCellCount
	 * = 0; }
	 */
	public void shoot() {
	 ...
	 * scheduler run mLowSensorCur = mLowSensor.get(); if (startDelay) { if
	 * (mTimer.get() - elapsed > OpConstants.kSeqIntakeDelay) {
	 * mTalonSeq.setSpeed(0); startDelay = false; } } }
	 * 
	 * public void addPowerCell() { if (mLowSensorCur) { startDelay = true; elapsed
	 * = mTimer.get(); // incr count at end of intaking powercell if
	 * (!mLowSensorLast) { mPowerCellCount++; } } else { if (mPowerCellCount <
	 * OpConstants.kMaxPowerCells) {
	 * mTalonSeq.setSpeed(OpConstants.kMotorSeqFwdIntakeSpeed); // incr count at
	 * beginning of intaking powercell //if (mLowSensorLast) { // mPowerCellCount++;
