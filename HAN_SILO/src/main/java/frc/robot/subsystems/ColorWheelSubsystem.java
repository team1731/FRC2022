...
public class ColorWheelSubsystem extends SubsystemBase {
	...
	public static final int[] colorSeq = { OpConstants.kWheelRed, OpConstants.kWheelGreen, OpConstants.kWheelBlue,
			OpConstants.kWheelYellow, OpConstants.kWheelRed, OpConstants.kWheelGreen };

	public enum WheelState {
		IDLE, // doing nothing ... waiting
		PREPARE, // verify we are reading the color wheel, i.e. motor is close enough to engage
					// wheel
		...
		COUNT, // 1) read color 2) incr count if color == sample 3) check if count is >= 6, 4)
				// signal led strip
	...
	public ColorWheelSubsystem() {
		...
		mColorWheelSolenoid = Constants.makeDoubleSolenoidForIds(1, OpConstants.k1ColorWheelRetract,
				OpConstants.k1ColorWheelExtend);
	...
	private void handleColorWheel(WheelMode mode) {
		...
		switch (mWheelState) {
			case IDLE:
				break;
			case PREPARE: // verify we are reading the color wheel, i.e. motor is close enough to engage
							// wheel
				if (checkSensor()) {
			...
			case START: // 1) read color 2) determine next color set as sample 3) start motor
				...
				break;
			case COUNT: // 1) read color 2) incr count if color == sample 3) check if count is >= 6, 4)
						// signal led strip
				zColor = getMatch();
	...
	/*
	 * private Color getColor() { //Color detectedColor = mColorSensor.getColor();
	 * //The sensor returns a raw IR value of the infrared light detected. //double
	 * IR = mColorSensor.getIR(); return mColorSensor.getColor(); }
	 */
	...
	private int getGameColor() {
		...
		if (gameData.length() > 0) {
			switch (gameData.charAt(0)) {
				case 'B':
					color = OpConstants.kWheelBlue;
					break; // Blue case code
				case 'G':
					color = OpConstants.kWheelGreen;
					break; // Green case code
				case 'R':
					color = OpConstants.kWheelRed;
					break; // Red case code
				case 'Y':
					color = OpConstants.kWheelYellow;
					break; // Yellow case code
				default:
					color = OpConstants.kWheelUnknown;
					break; // This is corrupt data
			}
		} else {
