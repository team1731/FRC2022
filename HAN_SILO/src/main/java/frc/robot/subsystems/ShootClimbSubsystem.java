...
public class ShootClimbSubsystem extends SubsystemBase {
	...
	public ShootClimbSubsystem(LedStringSubsystem m_ledstring) {
		...
		mTalonShoot1.configSelectedFeedbackSensor(FeedbackDevice.IntegratedSensor, OpConstants.kPIDLoopIdx,
				OpConstants.kTimeoutMs);
		mTalonShoot2.configSelectedFeedbackSensor(FeedbackDevice.IntegratedSensor, OpConstants.kPIDLoopIdx,
				OpConstants.kTimeoutMs);
		/**
	...
	public boolean atTargetVelocity() {
		return mTalonShoot1.getSelectedSensorVelocity() >= targetVelocity_UnitsPer100ms * 0.95
				&& mTalonShoot1.getSelectedSensorVelocity() < targetVelocity_UnitsPer100ms * 1.05;
	}
	...
	public void spinShooter(double shootMotorPercent_0_to_1) {
		 ...
		 * 2048 == max speed in ticks per 100ms ==> shootPercent is 0 to 1, so 100% ==
	...
	/*
	 * private void spinUpShooter(double desiredVelocity){ for(int i = 0; i < 100; i
	 * += 10){ mTalonShoot1.set(ControlMode.Velocity, desiredVelocity*i/100);
	 * mTalonShoot2.set(ControlMode.Velocity, desiredVelocity*i/100); try{
	 * Thread.sleep(500); } catch (InterruptedException e){
	 * System.err.println("Interrupted while sleeping. Don't wake me up pls");
	 * e.printStackTrace(); } } mTalonShoot1.set(ControlMode.Velocity,
	 * desiredVelocity); mTalonShoot2.set(ControlMode.Velocity, desiredVelocity); }
	 */
...
/*
 * public void modeShoot() { modeClimbing = false; //mTalonShoot.setSpeed(0.5);
 * mTalonShoot1.set(ControlMode.PercentOutput,OpConstants.kMotorShootPercent);
 * //mTalonShoot2.set(ControlMode.PercentOutput,OpConstants.kMotorShootPercent);
 * }
 * 
 * public void modeClimb() { modeClimbing = true; //mTalonShoot.setSpeed(-0.0);
 * // for testing only
 * mTalonShoot1.set(ControlMode.PercentOutput,OpConstants.kMotorShootPercent);
 * //mTalonShoot2.set(ControlMode.PercentOutput, 0); }
 * 
