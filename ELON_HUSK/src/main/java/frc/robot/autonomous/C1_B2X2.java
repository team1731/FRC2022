...
public class C1_B2X2 extends _DelayableStrafingAutoMode {
	...
	public C1_B2X2(DriveSubsystem m_robotDrive, IntakeSubsystem m_intake2, LaunchSubsystem m_launch2) {
		...
		SequentialCommandGroup commandGroup = new SequentialCommandGroup(
			...
			new LaunchCommandStart(m_launch,.4,true).raceWith(createSwerveCommand(m_robotDrive, "C1-1", 43, trajectory0,false)), // Drive to first ball	
			new LaunchCommandStart(m_launch,0.4,true).withTimeout(1),
			new LaunchCommandStart(m_launch,.4,true).raceWith(createSwerveCommand(m_robotDrive, "C1-2", 43, trajectory1,false)), // Drive to first ball	
			new LaunchBallCommandStart(m_launch),
			new LaunchCommandStart(m_launch,0.4,true).withTimeout(2),
