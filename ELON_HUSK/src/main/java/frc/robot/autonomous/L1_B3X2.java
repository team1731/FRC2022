...
public class L1_B3X2 extends _DelayableStrafingAutoMode {
	...
	public L1_B3X2(DriveSubsystem m_robotDrive, IntakeSubsystem m_intake2, LaunchSubsystem m_launch2) {
		...
		SequentialCommandGroup commandGroup = new SequentialCommandGroup(
			...
			new LaunchCommandStart(m_launch,0.39,true).raceWith(new RightIntakeCommand(m_intake)),
			new LaunchCommandStart(m_launch,0.39,true).raceWith(createSwerveCommand(m_robotDrive, "L4_B3L2_B5B4L2", -40.0, trajectory0,false)), // Drive to first ball	
			new LaunchCommandStart(m_launch,.39,true).raceWith(createSwerveCommand(m_robotDrive, "L4_B3L2_B5B4L2", -40.0, trajectory1,false)), // Drive to first ball
            new LaunchBallCommandStart(m_launch),
			new LaunchCommandStart(m_launch,0.39,true).withTimeout(2),
			new LaunchBallCommandStop(m_launch),
